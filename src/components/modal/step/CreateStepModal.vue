    ...
    setup(props, { emit }) {
      ...
      const submitForm = async () => {
        ...
        try {
          ...
          // Эмитируем событие с новым шагом
          emit('step-created', newStep);
          // Закрываем модальное окно
          emit('close');
          // Сброс формы
          resetForm();
        } catch (err) {
          error.value = err.message || 'Произошла ошибка при создании шага.';
          console.error('CreateStepModal - submitForm error:', err);
        } finally {
          loading.value = false;
        }
      };
